...
import './styles.css';
import { ReviewContext, GlobalDataContext, HelperContext, VocabulariesContext } from '../../../../context';
import configuration from './configuration';
import rotationsConfiguration from './components/Rotations/configuration';
import DescriptionBox from '../../../../components/DescriptionBox';
import designConfiguration from '../ExperimentsCommon/ExperimentalDesign/configurationCrop';
...
const Experiments = () => {
	...
	const populateRow = (rowMap) => {
		const configs = [configuration, designConfiguration, rotationsConfiguration];
	...
	const addExperiment = () => {
		if (experimentName !== '' && category !== '') {
			...
			updateCustomTerms(customTerms, setCustomTerms, 'add', `${configuration.id}_${id}`, configuration.content[1], category, vocabulariesContextValue.vocabs.experiment_category);
	...
	const editExperiment = () => {
		if (experimentName !== '' && category !== '') {
			...
			updateCustomTerms(customTerms, setCustomTerms, 'add', `${configuration.id}_${currentItem}`, configuration.content[1], category, vocabulariesContextValue.vocabs.experiment_category);
			setExperimentName('');
			setCategory('');
			setAddExperimentDialog(false);
			setCurrentItem(null);
	...
	const categoryTemplate = (data) => {
		return (
			<div>
				{translate(t, data.value.get('12841f10-62c6-439d-9290-4938ab1c124b').value.term)}
			</div>
		);
	};
	...
	const renderBody = () => {
		...
		return <ExperimentSteps selectedExperiment={selectedExperiment} setSelectedExperiment={setSelectedExperiment} stepValues={localFields} setStepValues={setLocalFields} compositeId={`${configuration.id}_${selectedExperiment}`} />;
	};
