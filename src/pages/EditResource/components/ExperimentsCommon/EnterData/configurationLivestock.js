const configurationLivestock = {
	...
	columns: [
		...
		{ field: 'product', header: 'Product Name', info: '', custom: true },
	...
	content: [
				...
				if (stepValues.size > 0) {
					...
					if (treatments.size > 0) {
						treatmentIds?.forEach((id) => {
							...
							const sites = treatment?.get('4f1cbd0c-92b7-44d1-b0d9-a844980bf061');
				...
				if (stepValues.size > 0) {
					...
					if (treatments.size > 0) {
						treatmentIds?.forEach((id) => {
							...
							if (treatment) {
								options = Array.from(treatment.get('herd-index').keys());
							}
						});
					}
		...
		// },
		{
			id: '523ea48a-0bd3-4fb9-a9bc-ee3c156f78e3',
			type: 'list',
			valid: false,
			data: true,
		...
		{
			id: '66a95135-806e-4d65-9826-cd15fa85cb7c',
			type: 'text',
			label: '',
			fieldInformation: '',
			valid: false,
